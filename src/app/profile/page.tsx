...
export default function ProfilePage() {
  ...
  const saveAddress = async (values: AddressInput) => {
    ...
    try {
      if (editing) {
        await api.updateAddress(token, editing.id, values);
      } else {
        await api.createAddress(token, values);
      }
  ...
  const deleteAddress = async (addr: Address) => {
    ...
    setMutationError(null);
    try {
      await api.deleteAddress(token, addr.id);
      await refreshProfile();
    } catch (err) {
      console.error(err);
  ...
  const setDefault = async (addr: Address) => {
    ...
    setMutationError(null);
    try {
      await api.updateAddress(token, addr.id, { ...toInput(addr), is_default: true });
      await refreshProfile();
    } catch (err) {
      console.error(err);
  ...
  if (!profile || !token) {
    return (
      <main className="space-y-4">
        <p className="text-sm text-gray-500">Профиль будет доступен после авторизации в Telegram.</p>
      </main>
    );
  }
  ...
  return (
    <main className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Данные получены из Telegram initData и вашего аккаунта.</p>
      </div>
